...
/* =============================================================================
   ...
   -----------------------------------------------------------------------------
   File mandiri: MODEL 3D (build) + ANIMASI (animate). Herbal adalah
   tabib yang memulihkan HP, memegang keranjang herbal + sabit. Badan jubah & animasi sama persis dengan
...
const NPC_Herbal={
  ...
  build(){
    ...
    const ROBE=0x7a4f7d,HOOD=0x4d3050,
    ...
    /* --- ALAT / SENJATA PERAN --- */
                /* keranjang herbal + sabit kecil */
    grip.add(at(box(0.24,0.18,0.20,WOODC),0,-0.14,0));
    grip.add(at(box(0.26,0.04,0.22,WOOD_D),0,-0.05,0));
    grip.add(at(box(0.18,0.09,0.16,0x6bbf4a),0,-0.03,0));
    grip.add(at(box(0.10,0.06,0.10,0x4f9a33),0.05,-0.01,0.03));
    const sickle=new THREE.Group();sickle.position.set(0.10,-0.02,0.10);
    sickle.add(at(box(0.04,0.22,0.04,WOOD_D),0,-0.08,0));
    const curve=box(0.16,0.04,0.03,METAL);curve.position.set(0.06,0.03,0);curve.rotation.z=0.5;sickle.add(curve);
    grip.add(sickle);
    ROT(grip,0.12,0,0.06);
