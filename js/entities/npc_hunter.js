...
/* =============================================================================
   ENTITAS NPC: PEMBURU (🏹)
   -----------------------------------------------------------------------------
   File mandiri: MODEL 3D (build) + ANIMASI (animate). Hunter adalah
   pemburu yang bisa direkrut, memegang busur + anak panah. Badan jubah & animasi sama persis dengan
...
const NPC_Hunter={
  ...
  build(){
    ...
    const ROBE=0x4f6b34,HOOD=0x2f4020,
    ...
    bow.add(at(box(0.05,0.34,0.05,WOODC),0,0.18,0));
    bow.add(at(box(0.05,0.34,0.05,WOODC),0,-0.18,0));
    const tipT=box(0.045,0.16,0.045,WOOD_D);tipT.position.set(0,0.40,0);tipT.rotation.z=0.22;bow.add(tipT);
    const tipB=box(0.045,0.16,0.045,WOOD_D);tipB.position.set(0,-0.40,0);tipB.rotation.z=-0.22;bow.add(tipB);
    bow.add(at(box(0.02,0.78,0.02,0xe8e2d2),0.05,0,0));
    grip.add(bow);
    const arrow=new THREE.Group();arrow.position.set(0.08,-0.05,0.03);
    arrow.add(at(box(0.025,0.50,0.025,WOOD_D),0,0,0));
    arrow.add(at(box(0.05,0.06,0.02,METAL),0,0.27,0));
    arrow.add(at(box(0.04,0.08,0.015,0xe8e2d2),0,-0.24,0));
    grip.add(arrow);
    ROT(grip,0.10,0,0.14);
