...
const CharView={
  ...
  BADGES:[
    {id:'god_of_death',title:'God of Death',icon:'<img src="buttons/sword_reaper.png" class="badge-custom-icon" alt="Death">',tag:'Mythic',desc:'Bantai 500 mob Reaper di dunia kegelapan (Syarat Rekrut Lich)',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('reaper')>=500),progress:()=>`${Math.min(500,(typeof RPG!=='undefined'?RPG.getMobKills('reaper'):0))}/500`},
    {id:'dragon_slayer',title:'Dragon Slayer',icon:'<img src="buttons/sword_dragon.png" class="badge-custom-icon" alt="Dragon">',tag:'Mythic',desc:'Tumbangkan 300 Naga Kuno di angkasa',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('dragon')>=300),progress:()=>`${Math.min(300,(typeof RPG!=='undefined'?RPG.getMobKills('dragon'):0))}/300`},
    {id:'high_king',title:'High King',icon:'<svg viewBox="0 0 24 24" class="badge-custom-icon"><path fill="#ffd23f" d="M3 19h18v2H3v-2zm1-2l2-10 4.5 5 3.5-7 3.5 7 4.5-5 2 10H4z"/></svg>',tag:'Royal',desc:'Duduk di singgasana tahta agung kastil kerajaan',check:()=>(typeof RPG!=='undefined'&&RPG.hasBadge('high_king')),progress:()=>(typeof RPG!=='undefined'&&RPG.hasBadge('high_king'))?'1/1':'0/1'},
    {id:'dragon_rider',title:'Dragon Rider',icon:'<img src="buttons/saddle.png" class="badge-custom-icon" alt="Rider">',tag:'Legend',desc:'Terbang mengarungi cakrawala bersama Naga peliharaan',check:()=>(typeof RPG!=='undefined'&&RPG.hasBadge('dragon_rider')),progress:()=>(typeof RPG!=='undefined'&&RPG.hasBadge('dragon_rider'))?'1/1':'0/1'},
    {id:'village_founder',title:'Village Founder',icon:'<img src="buttons/f_castle1.png" class="badge-custom-icon" alt="Castle">',tag:'Royal',desc:'Bangun teritori kastil dan aula desa hingga diakui penduduk',check:()=>(typeof RPG!=='undefined'&&RPG.hasBadge('village_founder')),progress:()=>(typeof RPG!=='undefined'&&RPG.hasBadge('village_founder'))?'1/1':'0/1'},
    {id:'first_blood',title:'First Blood',icon:'<img src="buttons/eff_bleed.png" class="badge-custom-icon" alt="Blood">',desc:'Kalahkan monster pertamamu di alam liar',check:()=>(typeof Player!=='undefined'&&(Player.kills||0)>=1),progress:()=>`${Math.min(1,typeof Player!=='undefined'?(Player.kills||0):0)}/1`},
    {id:'centurion',title:'Centurion',icon:'<img src="buttons/attack.png" class="badge-custom-icon" alt="Combat">',desc:'Kalahkan 100 monster di medan tempur',check:()=>(typeof Player!=='undefined'&&(Player.kills||0)>=100),progress:()=>`${Math.min(100,typeof Player!=='undefined'?(Player.kills||0):0)}/100`},
    {id:'monster_hunter',title:'Monster Hunter',icon:'<img src="buttons/sword_titan.png" class="badge-custom-icon" alt="Hunter">',desc:'Kalahkan 1.000 monster di seluruh biome',check:()=>(typeof Player!=='undefined'&&(Player.kills||0)>=1000),progress:()=>`${Math.min(1000,typeof Player!=='undefined'?(Player.kills||0):0)}/1000`},
    {id:'boss_slayer',title:'Apex Predator',icon:'<img src="buttons/boss_core.png" class="badge-custom-icon" alt="Apex">',desc:'Tumbangkan 50 Boss raksasa di dungeon dan alam liar',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('_boss')>=50),progress:()=>`${Math.min(50,typeof RPG!=='undefined'?RPG.getMobKills('_boss'):0)}/50`},
    {id:'slime_crusher',title:'Slime Crusher',icon:'<img src="buttons/gel.png" class="badge-custom-icon" alt="Slime">',desc:'Kalahkan 200 Slime hijau pembelah',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('slime')>=200),progress:()=>`${Math.min(200,typeof RPG!=='undefined'?RPG.getMobKills('slime'):0)}/200`},
    {id:'wolf_bane',title:'Alpha Wolf',icon:'<img src="buttons/cmeat.png" class="badge-custom-icon" alt="Wolf">',desc:'Kalahkan 150 Serigala malam',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('wolf')>=150),progress:()=>`${Math.min(150,typeof RPG!=='undefined'?RPG.getMobKills('wolf'):0)}/150`},
    {id:'golem_breaker',title:'Titan Breaker',icon:'<img src="buttons/stone.png" class="badge-custom-icon" alt="Golem">',desc:'Hancurkan 100 Golem batu peremuk tanah',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('golem')>=100),progress:()=>`${Math.min(100,typeof RPG!=='undefined'?RPG.getMobKills('golem'):0)}/100`},
    {id:'scorpion_hunter',title:'Venom Conqueror',icon:'<img src="buttons/eff_venom.png" class="badge-custom-icon" alt="Venom">',desc:'Taklukkan 150 Kalajengking gurun',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('scorpion')>=150),progress:()=>`${Math.min(150,typeof RPG!=='undefined'?RPG.getMobKills('scorpion'):0)}/150`},
    {id:'tarantula_bane',title:'Web Weaver',icon:'<img src="buttons/rope.png" class="badge-custom-icon" alt="Web">',desc:'Basmi 100 Tarantula pelompat',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('tarantula')>=100),progress:()=>`${Math.min(100,typeof RPG!=='undefined'?RPG.getMobKills('tarantula'):0)}/100`},
    {id:'centipede_bane',title:'Dune Leviathan',icon:'<img src="buttons/centipede_shell.png" class="badge-custom-icon" alt="Centipede">',desc:'Kalahkan 50 Kelabang raksasa tanah merah',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('kelabang')>=50),progress:()=>`${Math.min(50,typeof RPG!=='undefined'?RPG.getMobKills('kelabang'):0)}/50`},
    {id:'yeti_slayer',title:'Frost Lord',icon:'<img src="buttons/eff_frost.png" class="badge-custom-icon" alt="Yeti">',desc:'Tumbangkan 100 Yeti raksasa tundra salju',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('yeti')>=100),progress:()=>`${Math.min(100,typeof RPG!=='undefined'?RPG.getMobKills('yeti'):0)}/100`},
    {id:'trex_dominator',title:'Dino Tamer',icon:'<img src="buttons/hard_shell.png" class="badge-custom-icon" alt="T-Rex">',desc:'Tumbangkan 100 T-Rex buas pemangsa',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('trex')>=100),progress:()=>`${Math.min(100,typeof RPG!=='undefined'?RPG.getMobKills('trex'):0)}/100`},
    {id:'mammoth_hunter',title:'Colossus Hunter',icon:'<img src="buttons/plate_berserker.png" class="badge-custom-icon" alt="Mammoth">',desc:'Kalahkan 100 Mammoth bertaring raksasa',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('mammoth')>=100),progress:()=>`${Math.min(100,typeof RPG!=='undefined'?RPG.getMobKills('mammoth'):0)}/100`},
    {id:'angler_master',title:'Master Angler',icon:'<img src="buttons/fishing_rod.png" class="badge-custom-icon" alt="Angler">',desc:'Kalahkan / tangkap 100 ikan perairan',check:()=>(typeof RPG!=='undefined'&&RPG.getMobKills('fish')>=100),progress:()=>`${Math.min(100,typeof RPG!=='undefined'?RPG.getMobKills('fish'):0)}/100`},
    {id:'legendary_hero',title:'Legendary Hero',icon:'<svg viewBox="0 0 24 24" class="badge-custom-icon"><path fill="#ffd23f" d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>',tag:'Honor',desc:'Capai Level 50 sebagai petualang sejati Forecraft',check:()=>(typeof Player!=='undefined'&&(Player.level||1)>=50),progress:()=>`${Math.min(50,typeof Player!=='undefined'?(Player.level||1):1)}/50`}
  ],
  getBadgeName(id){
